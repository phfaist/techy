...
**Current Problem**: `Span` only tracks byte positions in the source string:
...
**Proposed Solution**: Rich source location tracking

```rust
/// A source identifies where content comes from
...
pub enum Source {
    ...
    /// Content from a URL
    Url(String),
    /// Content from a string (user-provided, no file)
    String {
...
pub struct SourceLocation {
    ...
    /// Byte range in that source
    pub span: Span,
    /// Optional: Line and column information (computed lazily)
    pub line_col: Option<(usize, usize)>,
}

impl SourceLocation {
    /// Get the actual text this location refers to
    pub fn text<'a>(&self, source_text: &'a str) -> &'a str {
        &source_text[self.span.start..self.span.end]
    }

    /// Compute line and column if not cached
    pub fn compute_line_col(&mut self, source_text: &str) {
        if self.line_col.is_none() {
            self.line_col = Some(compute_line_col(source_text, self.span.start));
        }
    }
        ...
        match &*self.source {
            Source::File { path, .. } => {
                if let Some((line, col)) = self.line_col {
                    ...
                    format!("{}:{}", path.display(), self.span.start)
                }
            }
            Source::Url(url) => format!("{}:{}", url, self.span.start),
            Source::String { name: Some(n) } => format!("{}:{}", n, self.span.start),
            Source::String { name: None } => format!("<string>:{}", self.span.start),
            Source::Synthetic { generator, .. } => {
                format!("<generated by {}>:{}", generator, self.span.start)
            }
        }
    }
}
