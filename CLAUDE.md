...
- **techy::serialize**: Serialization to and from a format-independent value model (SerialValue, SerialEntry, TableId, SerialIndex + `serial_index!`; the capability traits SerializableObject/DeserializableObject and SerializableValue/DeserializableValue with the SerializableLang opt-in; the engine SerdeSession, SerializeContext/DeserializeContext, ObjectSerdeDriver, TableHandle, Segment, DispatchingSerdeDriver/ObjectReader/IdentifierResolver; the standard-table drivers, typed positions, and by-kind extension traits; KnownProviders/ProviderRecipe, register_core_readers, DeserializedCondition; the errors; the optional `serde` cargo feature gates rendering only — `to_value`/`from_value`/`serial_bytes`); the preset's opt-in is `latexlike::serialize`
