...
- **techy::core::token**: The tokenization library (Tokenization/StdTokenization — the per-language bundle behind the `Token<L>`/`StreamPosition<L>` aliases, StdToken, TokenKind, TokenEdge; TokenReader, StdTokenReader; the scan helpers — `skip_whitespace`, `scan_paragraph_break`, `scan_group_delimiter`, `command_rule_at`, `scan_command`, `scan_comment`, `scan_specials_trigger` — and their match values GroupDelimiterMatch/CommandMatch/CommentMatch; TokenRules and its per-feature blocks with the matching *Overrides and the PrefixTable/TriggerChars caches; SpecialsMatch/SpecialsScanError; the TokenError family)
