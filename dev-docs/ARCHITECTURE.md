...
- **Structured conditions, not prose** ([§dd-dr:structured-diagnostics]): `Diagnostic`
  ...
  [§dd-dr:diagnostic-info-data-split], [§dd-dr:diagnostic-derive].
...
[§dd-dr:parse-traceback], [§dd-dr:refine-diagnostic-hook], [§dd-dr:diagnostics-retention].
...
- **The seed package `"base"`** ([§dd-dr:base-package]): pylatexenc's default specials
  as data (`&`, `~`, the typography ligatures — text-mode-only via per-entry
  visibility), plus the environment dispatch pair `begin`/`end` registered as ordinary
  entries ([§dd-dr:begin-end-dispatch]) — everything goes through the stack; nothing
  is hardcoded, everything is shadowable and unloadable.
