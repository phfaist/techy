...
| 6.7 | Closing docs, quarry, benchmark check | ✅ done (July 2026, phase-close session). NAMING_STRATEGY.md: `make_*` factory-hook principle + the §3.10 Phase 6 names in the core table + superseded plan-time names (`ContentParser`, `ParseOutcome`, enum-`TokenStopCondition`, `claim_post_space`, `peek_argument_token`). ARCHITECTURE.md §9: Phase 6 marked ✅ with the shipped/deferred split. DESIGN_RATIONALE.md: the factory entry's stale "check before Phase 6 closes" benchmark flag reconciled with the recorded deferral (consciously deferred, not dropped — the obligation stands open, unscheduled); the 6.6 report's flagged items recorded as §3.6 amendments (ctor `trigger_span` + always-required `invocation_name` + `with_match_invocation_name` switch on the terminator-parser entry; malformed-terminator consume = the command alone + stray-close-in-body unwinding on the mismatch entry; the Phase-7 `Invocation` lifetime finding on the factory entry) and the 6.4 module-doc takeover idiom rephrased positionally (`invocation_parser.rs`). §6 open questions checked: 1b/4/7/8 remain open, none invalidated by 6.1–6.6. Quarry: `src/parser/{mod,minparser}.rs` → `*_JUNK._rs` (rename per plan; note the user deleted the earlier junk files in b2c6ead — deleting these too is the user's call). Preliminary notes: already archived to docs/archive/ by the user. |
