...
**Status: EXECUTED — all subphases 7.0–7.9 done (July 2026); see §5 for the record.**
...
during subphase 7.0 — this file holds the working contract and the execution order. The sole
carried-over obligation is the §6.7 per-invocation-`Box` micro-benchmark (unscheduled, §1).
...
| 7.5 preset core | ✅ done (July 2026) | Checkpoint held (naming batch + user adjustments; DESIGN_RATIONALE §3.13): **single `Math` group class** (no `MathInline`/`MathDisplay` — display-ness is a delimiter fact: `NodeRef::math_style()` → `MathStyle::{Inline,Display}`), **no `Bracket` class / no `[]` default rule** (optional args use per-spec `temporary_groups`; verified — nothing else consumed them), **bare module-scoped enums** `GroupType::{Content,Math}` / `CallableType::{Macro,Environment,Specials}` / `Mode::{Text,Math}` (all `#[non_exhaustive]`, no crate-root re-exports), **seed package `"base"`** (user-named) with the full pylatexenc specials set (`& ~ `` '' -- --- !` ?`), **inherent** NodeRef sugar. Landed: `techy::latexlike` (`mod.rs`/`driver.rs`/`node_ref.rs`) — `Latexlike` (seed = `default_token_rules()` + `"base"`, `Mode::Text`; `scan_specials`/trigger chars = scope-stack folds; no `finalize_transition` override needed yet — mode consequences are all delta-borne so far), `LatexlikeDriver { recovery }` (`resolve_command` → stack as `Macro` with searched-providers miss detail; `group_interior_delta` math plug, unconditional on base mode), sugar `is_math_group`/`math_style`/`macro_name`/`environment_name`/`specials_name`. Tests: 21 + module doctest (defaults incl. `[]`-as-chars; math entry/exit, `$a$$b$` vs `$$…$$`, `{}` inherits math mode; mode-visible packages; specials longest-match, trigger-char declines, `"base"` unload; strict/tolerant unknown commands; seed-coherence pin). Docs: NAMING preset section, ARCHITECTURE §8 rewrite + §state stale-bullet fix. Deferred: type aliases (`LatexParseResult`…, bikeshed open), driver package-loading helpers (need a registry), `\begin`/`\end` + `MacroSpec`-family helpers (7.6). |
| 7.6 environments | ✅ done (July 2026) | Checkpoint held (5 decisions; DESIGN_RATIONALE §3.13 entries): (a) `\begin`/`\end` = ordinary `Macro` entries of `"base"` (`BeginSpec`/`EndSpec`), not driver code — composition stamps `CallableType::Environment` + the environment's name/spec on the node; (b) funnel per D4: concrete `EnvironmentSpec` wrapper (delegating `CallableSpec`, "environment ‘…’" frames) over dyn `EnvironmentBehavior` (defaulted `arguments`/`body_state_delta` (owned)/`make_body_parser`; hooks take a `#[non_exhaustive]` `EnvironmentInvocation` facts struct — parsed args deliberately left out until a consumer needs them); `.with_body_delta` = adapter-wrapping, total over custom behaviors; non-`EnvironmentSpec` registrations get default body handling; (c) `MacroSpec`/`SpecialsSpec` = real types with preset frame vocabulary (base specials switched to `SpecialsSpec`); (d) orphan `\end`: dispatch-time = orphan by construction; `OrphanEnd` + chars over the consumed `\end{name}` extent; (e) ids `latexlike.environments.*`, files `latexlike/environments.rs` + `latexlike/spec.rs`. Landed: composition promoted from the §G rehearsal (rigid name group → CT-Environment stack lookup → `parse_declared_arguments` under env-name frames → behavior body parser under body delta → `"body"` slot record); `UnknownEnvironment` tolerant fallback keeps parsing to the terminator; starred names as separate entries verified. Tests: 27 + module doctest (§G matrix incl. exact spans, nesting, body deltas (`nocomments`, `equation`+mode+visibility), recovery matrix under `Latexlike` (mismatch/EOF/malformed-terminator/stray-close/malformed-begin/orphan-end ×strict/tolerant), out-of-the-box begin, `Unload("base")` kills dispatch, expression-guard divergence, frame vocabulary, generic-spec-under-Environment) + 3 spec.rs tests. Note: chars fallbacks cover trigger post-space (partition invariant); pre-existing unused test import removed (latexlike/driver.rs). |
| 7.7 verbatim + arg codes | ✅ done (July 2026) | No checkpoint marked; in-flight decisions recorded (DESIGN_RATIONALE §3.13, three entries). Landed: **`constructs::verbatim_parser`** — `verbatim_state_delta` (the pinned recipe as data), `VerbatimArgumentParser` (`\verb`: auto-matched/fixed/custom-table delimiters, paired-delimiter depth counter, group+chars shape with content = group children, raw chars recorded under the verbatim state, delimiter discovery under a whitespace-only probe delta with the inherited expected close **cleared**), `VerbatimBodyParser` (literal-terminator environment contents, produces `EnvironmentBody`), conditions `UnterminatedVerbatim`/`ExpectedVerbatimDelimiter`; **`EnvironmentBody.content`** (new field, `Copy` dropped) — the body parser designates the slot content, so the gobbled `\begin{verbatim}` newline is *staged but designated out* (keeps invariants 2+3 exactly, incl. the lstlisting args-before-body shape; compositions switched); **`GroupArgumentParser::with_rule`** — the mandatory minted-rule form (`r<c1><c2>`), temporary-rule scoping like the optional parser, **no expression fallback**; **preset** — `GroupType::Verbatim` (the anticipated 7.5 slot; no `Mode::Verbatim` — rules-borne), `VerbatimBehavior` (any-name terminator back-reference; arguments parse tokenized ahead of the raw body), `argument_specs`/`ArgumentCodeError` (N8 factory: whitespace-separated codes, `v` takes 2 delimiter chars exactly when directly followed by non-whitespace; `m`→`GroupArgumentParser` — parity-table row refined; `o`/`d` unwrap lone protective `{…}`). Tests: 39 new + 2 doctests — pylatexenc `test_latexnodes_parsers_verbatim.py` ported whole (delimited + environment-contents, spans exact, features-off state pinned) and the `_stdarg.py` slice (m incl. comment noise, o incl. `[{a]b}]` unwrap, s/t, r incl. nesting/brace-protection/no-fallback, d, v auto+fixed), plus techy recovery matrices (strict/tolerant: missing delimiter, wrong fixed delimiter, unterminated `\verb`, unterminated environment), `\verb`-inside-group expected-close interplay, literal-terminator rigidity (`\end {verbatim}` ≠ terminator), no-nesting, `verbatim*`, empty/newline-only bodies; `check_tree_invariants` throughout; all prior suites green (461 lib tests), `cargo doc` clean. Docs: ParserLibraryParity N7/N8 → implemented (m-row refinement noted), NAMING verbatim rows + preset updates, ARCHITECTURE §constructs/§8 amendments. |
| 7.8 view API | ✅ done (July 2026) | Checkpoint held (multi-round shape session; DESIGN_RATIONALE §3.4 + §3.5 entries + two settle-notes): **`NodeSlice`** (Copy `{tree, range}`; user-driven: exact `span()`/`source_text()` in return types — `Option`, `None` iff empty/mixed-source) returned by `children()` + region/content accessors + new `_named` trio; helpers as **free functions in `node::extract`** (user alternative over method-hanging; readers take iterators, builders take `NodeSlice` — empty slices need the tree anchor); **builder route** for split output (user choice over the sketched ad-hoc-`NodeRef` enum, analyzed and recorded): `split_at_chars`/`parse_keyval` mint real trees via new crate-internal `node::copy::copy_subtree_into` (resolved regions re-staged; the §3.5 cross-tree revisit clause's first machinery), boundary partials = fresh span-backed `Chars` (owned-content partials keep whole-node provenance); `Split`/`KeyVals` own their tree privately behind primary accessors (`segment(i)`/`segments()`, `keyval(i)`/`get`/`iter`, user requirement) returning `NodeSlice` views. Keyval **no-knobs** (source-ordered dup-preserving entries, `get` = last-wins, `value: Option` sharper than pylatexenc, `value_content()` unwrap accessor, keys trimmed — recorded deviation, `get_combined_with(key, sep)` with synthesized separator nodes). `content_as_chars` strict `Result<Cow>` (no tolerance — read-time). `descendants()` preorder on `NodeRef` + `NodeTree` sugar. **`iter_symbols(callable_type, mode)`** — required type filter (user; specials enumerate under their recorded type, trigger as name), mode not state (visibility is mode-determined, verified), `None` = unenumerable (fallback), borrowed `SymbolEntry`; stack dedup first-visible-wins = `retrieve_spec` order. **`ClosedVocabulary`** (`const ALL`) opt-in in `state` — *not* a `Lang` bound (SimpleLang `u32` ids unlistable; deviation from the session sketch, reasoned); impls for `()` + the preset trio. Call-site migration: ~62 sites gained `.iter()`. Tests: +39 lib (slice spans/iteration, document order, named accessors, copy incl. nested designations + full invariants on pure copies, split/keyval/combined end-to-end over `Latexlike` incl. materialized-tree partials, symbol enumeration incl. base-package visibility) + extract module doctest; 500 lib tests green, `cargo doc` clean, zero warnings. Docs: NAMING new+superseded rows, ParserLibraryParity N5 implemented. |
| 7.9 acceptance suite | ✅ done (July 2026) | Design round held before coding (user decisions): **integration-crate placement** — `techy/tests/acceptance.rs`, public-API-only ("anything the port cannot reach is an API gap"), with a **helper-dedup mandate** → promotions `NodeRef::summary()` (core, Lang-generic; format documented non-contractual) + `Language::with_provider` (fallible seed sugar), `test_support.rs` shrunk to thin wiring; **descriptive test names** + `pylatexenc:` provenance comments, quirk-tied tests omitted with reasons (skip table in the file header: token-layer, construct-level, `read_max_nodes`-dependent, specials-verbatim, `\newcommand`); **`\n\n`-as-specials = driver flag** `ParagraphBreakStyle` (`Chars` default / `Specials` — user design over package-correlated probing: the tokenizer detects breaks before the specials scan, so package `"\n\n"` entries are dead data; enum form approved over the boolean sketch; canonical name `"\n\n"` + actual-run span, per-break spec keeps the driver `Copy`); **`FallbackProvider` for `test_errors`** (bottom of a `ReplaceStack`-built stack; pylatexenc unknown-macro parity + fallback acceptance exercise); **§6.7 micro-benchmark deferred** (stays unscheduled); **guide**: new `docs/learn-by-example.md` chapter (user: generous, suite-inspired) wired as `guide::learn_by_example` + sidebar `GUIDE_PAGES` — 17 compile-checked doctests; argument-code call sites **list-shaped** (`args(&["o","m"])`, one joining helper) anticipating the factory's planned list signature (the `### PHF` note). Landed: the suite — 30 tests over a shared test-side `testdb` (macros/environments incl. math-body deltas, `\text`/`\mbox` per-argument text-mode deltas = pylatexenc `args_math_mode` via `ArgumentSpec::with_state_delta`, `\verb`/`verbatim`/`lstlisting`), span-exact `{range} {summary}` outlines, every happy path parsed in **both** recovery modes with tree-identity asserted (`parse_ok`), error matrices per-mode, `check_tree_invariants` on every parse; ports: maybe_optional_arg, environment (enumerate block), get_latex_nodes (sliced + cite-4×m shadowing), comments (both documents), mathmodes (full, incl. recorded-mode asserts) + dollardollar **including the tail pylatexenc commented out**, verbatim + lstlisting ×2, errors (strict aborts / tolerant completes), dangling-args 0/1/2 (2 = custom `_` specials with `m`), invalid-environment 0/1/2, issue #49 (span-exact adaptation) / #57. **Found & fixed via the invariant gate:** 7.4's "accepted byte-accounting break" — the root stray-close skip dropped the delimiter's bytes, failing the partition invariant on the `test_errors` document; the skip now stages the consumed delimiter as a `Chars` node (the markup-in-chars recovery artifact; language.rs + 7.4/7.6 test updates; DESIGN_RATIONALE §3.6 settle-note). Docs: DESIGN_RATIONALE entries (¶-break flag, both promotions, suite conventions, stray-close amendment), NAMING rows (`ParagraphBreakStyle`, `summary` — "summary" over "shape", `with_provider`), ParserLibraryParity N1 → implemented+verified, ARCHITECTURE §9 Phase 7 ✅. Verification: 502 lib + 30 acceptance + 25 doctests green, `cargo docs` zero warnings. Phase 7 exit criteria met; §6.7 benchmark the sole carried-over obligation. |
