...
//! information (Phase 7.8, DESIGN_RATIONALE.md §3.5).
//!
...
//! span-contiguous by the §nodes partition invariant, so the covering span is the first
...
impl<'t, L: Lang> NodeSlice<'t, L> {
    ...
    /// (the §nodes partition invariant), so this is the first node's start to the last
