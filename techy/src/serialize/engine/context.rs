//! The contexts a serialization or deserialization call receives:
//! [`SerializeContext`] and [`DeserializeContext`], a call's access to the session in
//! progress.
...
/// A serialization call's access to the session in progress.
///
/// This is the `cx` argument of
...
/// and [`ObjectSerdeDriver::serialize_object`]. Through it a call interns the objects
/// the one being serialized refers to ([`intern`](SerializeContext::intern)), reads the
/// caller's user data, and finds tables by name.
///
...
/// support serialization. It is constructed by the session only, for the duration of one
/// call.
...
impl<'a, L: SerializableLang> SerializeContext<'a, L> {
    ...
    /// Interns `object` into the table `table`, returning its typed position — the
    /// object's serialized reference.
    ///
    /// An object already interned in the table — the same `Arc`, by pointer identity —
    /// yields its existing position without being serialized again, which is what
    /// preserves sharing. A new object is serialized by the table's driver (which may
    /// intern the objects it refers to, recursively) and appended to the table.
    ///
    /// # Errors
    ///
    ...
    /// ([`SerializeError::DescentLimitExceeded`]); the entry has an identifier other
    /// than a homogeneous table's ([`SerializeError::UnexpectedIdentifier`]) or
    ...
    /// `D` (see [`SerdeSession::table_handle`]).
    ///
...
/// A deserialization call's access to the session in progress.
///
/// This is the `cx` argument of
...
/// Through it a call reads the objects the one being rebuilt refers to
/// ([`object`](DeserializeContext::object)), reaches the reading environment in the
/// caller's user data, and finds tables by name.
///
/// A context exists only for a [`SerializableLang`]: that bound is what makes the
...
impl<'a, L: SerializableLang> DeserializeContext<'a, L> {
    ...
    /// Returns the object stored at position `index` of table `table`, rebuilt from its
    /// entry if it has not been rebuilt yet.
    ///
    /// A segment's entries are deserialized in table and position order, and any entry
    /// referred to before its turn is deserialized on demand. The same position always
    /// yields the same `Arc`, so sharing survives the round trip.
    ///
    /// The position is one of this session's own — typically read out of the entry being
    /// deserialized, whose table references the session has already translated into its
    /// own numbering.
    ///
    /// # Errors
    ///
    ...
    /// [`SerdeSession::set_user_data`] (one value per type).
    ///
    /// This is where the *reading environment* is found: the live objects that
    /// serialized data refers to by identity rather than describing in full — the
    /// crate's own providers through
    /// [`KnownProviders`](crate::serialize::KnownProviders), a framework's through its
    /// own environment type.
    pub fn user_data<T: Any>(&self) -> Option<&T> {
        self.session.user_data::<T>()
    }
